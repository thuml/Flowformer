...
def kernel_feature_map(x):
    return nn.relu(x) + 1e-6


def flow_attention(query,
                   key,
                   value,
    ...
    sink_incoming = 1.0 / (
        jnp.einsum('nlhd,nhd->nlh', query_mapped + eps, jnp.sum(key_mapped, axis=1) + eps))
    source_outgoing = 1.0 / (
        jnp.einsum('nshd,nhd->nsh', key_mapped + eps, jnp.sum(query_mapped, axis=1) + eps))

    conserved_sink = jnp.einsum('nlhd,nhd->nlh', query_mapped + eps,
                                jnp.sum(key_mapped * source_outgoing[:, :, :, None], axis=1) + eps)
    conserved_source = jnp.einsum('nshd,nhd->nsh', key_mapped + eps,
                                  jnp.sum(query_mapped * sink_incoming[:, :, :, None], axis=1) + eps)

    sink_allocation = nn.sigmoid(conserved_sink * (float(query_mapped.shape[1]) / float(key_mapped.shape[1])))
    source_competition = nn.softmax(conserved_source, axis=-1) * key_mapped.shape[1]
    kv = jnp.einsum('nshd,nshm->nhdm', key_mapped, value * source_competition[:, :, :, None])
    y = jnp.einsum('nlhd,nhdm->nlhm', query_mapped, kv)
    y = (y * sink_incoming[:, :, :, None]) * sink_allocation[:, :, :, None]
    return y
