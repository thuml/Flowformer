...
from lra_benchmarks.models.flowformer import flowformer
import numpy as onp
...
def get_model(model_type, create_model_fn, model_kwargs, *create_model_args):
    ...
    if model_type == 'flowformer':
        return create_model_fn(flowformer.LinearTransformerEncoder,
                               model_kwargs, *create_model_args)
    if model_type == 'flowformer_retrieval':
        return create_model_fn(flowformer.LinearTransformerDualEncoder,
                               model_kwargs, *create_model_args)
    else:
