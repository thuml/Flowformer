...
from flowformergpt.utils import CfgNode as CN
...
class FlowFormer_GPT(nn.Module):
    """ GPT Language Model """
    ...
    def __init__(self, config):
        ...
        if type_given:
            ...
            config.merge_from_dict({
                                       ...
                                       # GPT-1
                                       'flowformer-openai-gpt': dict(n_layer=12, n_head=12, n_embd=768),  # 117M params
                                       # GPT-2 configs
                                       'flowformer-gpt2': dict(n_layer=12, n_head=12, n_embd=768),  # 124M params
                                       'flowformer-gpt2-medium': dict(n_layer=24, n_head=16, n_embd=1024),
                                       # 350M params
                                       'flowformer-gpt2-large': dict(n_layer=36, n_head=20, n_embd=1280),  # 774M params
                                       'flowformer-gpt2-xl': dict(n_layer=48, n_head=25, n_embd=1600),  # 1558M params
                                       # Gophers
                                       'flowformer-gopher-44m': dict(n_layer=8, n_head=16, n_embd=512),
                                       # (there are a number more...)
                                       # I made these tiny models up
                                       'flowformer-gpt-mini': dict(n_layer=6, n_head=6, n_embd=192),
                                       'flowformer-gpt-micro': dict(n_layer=4, n_head=4, n_embd=128),
                                       'flowformer-gpt-nano': dict(n_layer=3, n_head=3, n_embd=48),
                                   }[config.model_type])
    ...
    def from_pretrained(cls, model_type):
        ...
        """
        assert model_type in {'flowformer-gpt2', 'flowformer-gpt2-medium', 'flowformer-gpt2-large',
                              'flowformer-gpt2-xl'}
